...
const serviceUrls = {
  auth: (config.authServiceUrl || 'http://localhost:3001').replace(/\/$/, ''),
  task: (config.taskServiceUrl || 'http://localhost:3002').replace(/\/$/, ''),
};
...
function updateWelcome() {
  if (state.user) {
    ...
    elements.welcomeSubtitle.textContent = 'Your session is issued by auth-service and your todos come from task-service.';
...
function renderTodos() {
  ...
  elements.todoEmpty.classList.toggle('hidden', state.token && todos.length > 0);
  ...
  todos.forEach((todo) => {
    ...
    card.innerHTML = `
      ...
      <div class="todo-actions">
        <button class="todo-action" data-action="toggle" data-id="${todo._id}" type="button">${todo.completed ? 'Mark active' : 'Mark complete'}</button>
...
async function handleAuthSubmit(event) {
  ...
  try {
    ...
    const result = await apiFetch(serviceUrls.auth, path, {
      method: 'POST',
...
async function loadTodos() {
  ...
  try {
    ...
    const result = await apiFetch(serviceUrls.task, `/api/todos${query}`);
...
async function handleTodoSubmit(event) {
  ...
  try {
    const path = state.editingTodoId ? `/api/todos/${state.editingTodoId}` : '/api/todos';
    const method = state.editingTodoId ? 'PUT' : 'POST';
    const result = await apiFetch(serviceUrls.task, path, {
      method,
      body: JSON.stringify(payload),
    });
...
async function handleTodoAction(event) {
  ...
  const todo = state.todos.find((item) => item._id === button.dataset.id);
  if (!todo) {
    return;
  }
  ...
  try {
    ...
    if (button.dataset.action === 'toggle') {
      const result = await apiFetch(serviceUrls.task, `/api/todos/${todo._id}`, {
        method: 'PUT',
